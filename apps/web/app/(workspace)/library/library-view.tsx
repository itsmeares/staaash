...
export function LibraryView({
  ...
  const openItem = (id: string) => {
    ...
    if (file) {
      if (file.viewerKind) router.push(`/library/files/${file.id}`);
      else window.location.href = `/api/library/files/${file.id}/download`;
    }
  };
  ...
  const submitRename = async (id: string, kind: "folder" | "file") => {
    ...
    const endpoint =
      kind === "folder"
        ? `/api/library/folders/${id}/rename`
        : `/api/library/files/${id}/rename`;
  ...
  const toggleFavorite = async (
    ...
    const endpoint =
      kind === "folder"
        ? `/api/library/folders/${id}/favorite`
        : `/api/library/files/${id}/favorite`;
    await fetch(endpoint, {
      method: "POST",
  ...
  const trashItem = async (id: string, kind: "folder" | "file") => {
    const endpoint =
      kind === "folder"
        ? `/api/library/folders/${id}/trash`
        : `/api/library/files/${id}/trash`;
    await fetch(endpoint, {
      method: "POST",
